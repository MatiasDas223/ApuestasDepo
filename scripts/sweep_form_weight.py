"""
sweep_form_weight.py
Sweep FORM_WEIGHT y simula que ROI habria dado cada valor.
Para cada FORM_WEIGHT:
  - Walk-forward: excluye fixture del historico, recalcula prob con nuevo FW
  - Si new_edge >= MIN_EDGE, cuenta la bet con su resultado real
"""
import sys
import csv
import re
...
from backtest_v3 import _mercado_to_prob_key, _parse_prob

VB_CSV   = BASE / 'data/apuestas/value_bets.csv'
HIST_CSV = BASE / 'data/historico/partidos_historicos.csv'

FORM_WEIGHTS = [0.0, 0.10, 0.20, 0.30, 0.40]
MIN_EDGE = 0.04
N_SIM    = 20_000
...
def run_one_fw(fw_value, bets_by_fixture, hist_all):
    modelo_v3.FORM_WEIGHT = fw_value
    results = []  # (bet, new_prob, new_edge)
    for fid, bets in bets_by_fixture.items():
        ...
        try:
            ...
            print(f"  [FW={fw_value}][fid={fid}] err: {e}")
            continue
...
def summarize(fw, results):
    taken = [r for r in results if r['new_edge'] >= MIN_EDGE]
    if not taken:
        return {'fw': fw, 'N_total': 0, 'N_taken': 0, 'W': 0, 'L': 0, 'pnl': 0.0, 'roi': 0.0}
    ...
    roi = pnl / len(taken) * 100
    return {'fw': fw, 'N_total': len(results), 'N_taken': len(taken),
            'W': w, 'L': l, 'pnl': pnl, 'roi': roi, 'taken': taken}
...
def main():
    ...
    print(f"Cargando historico...")
    hist_all = load_csv(str(HIST_CSV))
    print(f"  {len(hist_all)} partidos")

    print(f"\nSweep FORM_WEIGHT = {FORM_WEIGHTS}")
    print(f"N_SIM={N_SIM}, MIN_EDGE={MIN_EDGE}\n")

    summaries = []
    all_taken = {}
    for fw in FORM_WEIGHTS:
        print(f"  [FW={fw}] corriendo...")
        results = run_one_fw(fw, bets_by_fixture, hist_all)
        s = summarize(fw, results)
        summaries.append(s)
        all_taken[fw] = s.get('taken', [])
        print(f"    N_taken={s['N_taken']}/{s['N_total']}  W={s['W']}  L={s['L']}  PnL={s['pnl']:+.2f}  ROI={s['roi']:+.1f}%")

    print(f"\n{'='*70}")
    print(f"  RESUMEN GLOBAL")
    print(f"{'='*70}")
    print(f"{'FW':>5s}  {'N bets':>8s}  {'W':>4s}  {'L':>4s}  {'PnL':>8s}  {'ROI':>8s}")
    for s in summaries:
        print(f"{s['fw']:>5.2f}  {s['N_taken']:>8d}  {s['W']:>4d}  {s['L']:>4d}  {s['pnl']:>+8.2f}  {s['roi']:>+7.1f}%")

    print(f"\n{'='*70}")
    print(f"  ROI por CATEGORIA x FORM_WEIGHT")
    print(f"{'='*70}")
    all_cats = set()
    cat_data = {}
    for fw in FORM_WEIGHTS:
        cb = category_breakdown(all_taken[fw])
        cat_data[fw] = cb
        all_cats.update(cb.keys())

    header = f"{'Categoria':>18s}" + "".join(f"  FW={fw:>4.2f}       " for fw in FORM_WEIGHTS)
    print(header)
    for cat in sorted(all_cats):
        row = f"{cat:>18s}"
        for fw in FORM_WEIGHTS:
            d = cat_data[fw].get(cat, {'n':0,'w':0,'pnl':0.0})
